...
import { checkUserSession, decideTransactionStatus } from '@/helpers/api/transactions-helper'
...
// TODO: Review this implementation to include pagination
...
export async function GET(req: NextRequest) {
    try {
        ...
        // Optional limit query (not a cursor pagination, just a cap)
        const url = new URL(req.url);
        const limitParam = Number(url.searchParams.get("limit") || MAX_LIMIT);
        const cap = Math.max(1, Math.min(limitParam, MAX_LIMIT));

        let q = adminDb.ref(basePath).orderByChild("createdAt").limitToLast(cap);
        ...
        if (snap.exists()) {
            ...
            transactions = raw.filter(
                (r) => r && typeof r.createdAt === "number" && typeof r.id === "string"
            ) as Tx[];
        }

        // Newest first
        transactions.sort((a, b) => (b.createdAt - a.createdAt) || (b.id > a.id ? 1 : -1));
